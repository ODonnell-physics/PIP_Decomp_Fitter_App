# -*- coding: utf-8 -*-
"""
Created on Thu Jan 13 10:57:40 2022
...
def model(t, a1, mu1, sig1, tau1, a2, mu2, sig2, tau2, a3, mu3, sig3, tau3, a4, mu4, sig4, tau4):
    try:
        measured_signal2
    except:
...
def p4_s(m_s,m_s_e,freqs,p_times,f_i,f_f,t_i,t_f,ig,ub,lb, plotter=True, f_unit="GHz", b_unit="SFU", t_unit="s", log_scale=False, rolling_ig=False):

    def model(t,a1,mu1,sig1,tau1,a2,mu2,sig2,tau2,a3,mu3,sig3,tau3,a4,mu4,sig4,tau4):#model_4pulse_seperate(t,a1,mu1,sig1,tau1,a2,mu2,sig2,tau2,a3,mu3,sig3,tau3,a4,mu4,sig4,tau4):
    ...
    if plotter:
        if not os.path.exists('./4_pulses_seperate_fit'):
            os.makedirs('./4_pulses_seperate_fit')

        pref = './4_pulses_seperate_fit/4_seperate'
    oddflag= False
    ...
    for f in range(f_i, f_f, 1):
        ...
        try:
            ...
            fit_a1,fit_mu1,fit_sig1,fit_tau1,fit_a2,fit_mu2,fit_sig2,fit_tau2,fit_a3,fit_mu3,fit_sig3,fit_tau3,fit_a4,fit_mu4,fit_sig4,fit_tau4 = ans
            fit_sa1,fit_smu1,fit_ssig1,fit_stau1,fit_sa2,fit_smu2,fit_ssig2,fit_stau2,fit_sa3,fit_smu3,fit_ssig3,fit_stau3,fit_sa4,fit_smu4,fit_ssig4,fit_stau4 = np.sqrt(np.diag(cov))
        
            
        
            results=np.array([fit_a1,fit_mu1,fit_sig1,fit_tau1,fit_a2,fit_mu2,fit_sig2,fit_tau2,fit_a3,fit_mu3,fit_sig3,fit_tau3,fit_a4,fit_mu4,fit_sig4,fit_tau4])
            results_error= np.array([fit_sa1,fit_smu1,fit_ssig1,fit_stau1,fit_sa2,fit_smu2,fit_ssig2,fit_stau2,fit_sa3,fit_smu3,fit_ssig3,fit_stau3,fit_sa4,fit_smu4,fit_ssig4,fit_stau4])
            ...
            current_mdl= model(time,fit_a1,fit_mu1,fit_sig1,fit_tau1,fit_a2,fit_mu2,fit_sig2,fit_tau2,fit_a3,fit_mu3,fit_sig3,fit_tau3,fit_a4,fit_mu4,fit_sig4,fit_tau4)[0:s_l]
            diff=measured_signal2- current_mdl
            xi1= (diff)**2/(l*measured_signal2e)
            #xi1[56:60]=0
            xi=np.sum(xi1)    
            xirs[f-f_i]=xi/float(s_l-len(ans))
            ...
            if plotter:
                ...
                ax.plot(model(time,fit_a1,fit_mu1,fit_sig1,fit_tau1,0,fit_mu2,fit_sig2,fit_tau2,0,fit_mu3,fit_sig3,fit_tau3,0,fit_mu4,fit_sig4,fit_tau4), label="Fit Flux P1", color='darkorange')
                ax.plot(model(time,0,fit_mu1,fit_sig1,fit_tau1,fit_a2,fit_mu2,fit_sig2,fit_tau2,0,fit_mu3,fit_sig3,fit_tau3,0,fit_mu4,fit_sig4,fit_tau4), label="Fit Flux P2", color='navy')
                ax.plot(model(time,0,fit_mu1,fit_sig1,fit_tau1,0,fit_mu2,fit_sig2,fit_tau2,fit_a3,fit_mu3,fit_sig3,fit_tau3,0,fit_mu4,fit_sig4,fit_tau4), label="Fit Flux P3", color='darkgreen')
                ax.plot(model(time,0,fit_mu1,fit_sig1,fit_tau1,0,fit_mu2,fit_sig2,fit_tau2,0,fit_mu3,fit_sig3,fit_tau3,fit_a4,fit_mu4,fit_sig4,fit_tau4), label="Fit Flux P4", color='dodgerblue')
    ...
    if plotter:
        try:
            ...
            lower_stop_tau = min(lb[3], lb[7], lb[11], lb[15])
            upper_stop_tau = 1.05 * max(ub[3], ub[7], ub[11], ub[15])
            ...
            lower_quartile_tau = max(np.nanpercentile(result[3, :], 25), np.nanpercentile(result[7, :], 25),
                                     np.nanpercentile(result[11, :], 25), np.nanpercentile(result[15, :], 25))
            upper_quartile_tau = max(np.nanpercentile(result[3, :], 75), np.nanpercentile(result[7, :], 75),
                                     np.nanpercentile(result[11, :], 75), np.nanpercentile(result[15, :], 75))
            ...
            avg_tau_e = max(np.nanpercentile(result_error[3, :], 50), np.nanpercentile(result_error[7, :], 50),
                            np.nanpercentile(result_error[11, :], 50), np.nanpercentile(result_error[15, :], 50))
            ...
            mid_tau_max = max([np.nanpercentile(result[3, :], 50), np.nanpercentile(result[7, :], 50),
                               np.nanpercentile(result[11, :], 50), np.nanpercentile(result[15, :], 50)])
            ...
            ax[1][1].errorbar(fl[f_i:f_f], result[15, 0:f_w], yerr=result_error[15, 0:f_w], fmt='.', label="Tau 4", color='dodgerblue')
            ...
            plt.savefig(pref + '_pulse4_seperateParameter_Panels_.png')
            plt.close()

            parms=[["freqs"],["fit_a1"], ["fit_mu1"], ["fit_sig1"], ["fit_tau1"],["fit_a2"], ["fit_mu2"], ["fit_sig2"], ["fit_tau2"], ["fit_a3"], ["fit_mu3"], ["fit_sig3"], ["fit_tau3"],["fit_a4"], ["fit_mu4"], ["fit_sig4"], ["fit_tau4"],["e_fit_a1"], ["e_fit_mu1"], ["e_fit_sig1"], ["e_fit_tau1"],["e_fit_a2"], ["e_fit_mu2"], ["e_fit_sig2"], ["e_fit_tau2"], ["e_fit_a3"], ["e_fit_mu3"], ["e_fit_sig3"], ["e_fit_tau3"],["e_fit_a4"], ["e_fit_mu4"], ["e_fit_sig4"], ["e_fit_tau4"],['reduced Xi^2']]
