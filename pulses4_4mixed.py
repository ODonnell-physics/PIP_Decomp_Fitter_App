# -*- coding: utf-8 -*-
"""
Created on Thu Jan 13 15:01:19 2022
...
def p4_4m(m_s,m_s_e,freqs,p_times,f_i,f_f,t_i,t_f,ig,ub,lb, plotter=True, f_unit="GHz", b_unit="SFU", t_unit="s", log_scale=False, rolling_ig=False):
    ...
    result=np.zeros([15,f_w])
    result_error=np.zeros([15,f_w])
    ...
    if plotter:
        if not os.path.exists('./4_pulses_4_mixed_fit'):
            os.makedirs('./4_pulses_4_mixed_fit')

        pref = './4_pulses_4_mixed_fit/4_4'
    ...
    for f in range(f_i, f_f, 1):
        ...
        try:
            ...
            results=np.array([fit_a1,fit_mu1,fit_sig1,fit_tau1,fit_a2,fit_mu2,fit_sig2,fit_tau2,fit_a3,fit_mu3,fit_sig3,fit_a4,fit_mu4,fit_sig4,fit_w])
            results_error= np.array([fit_sa1,fit_smu1,fit_ssig1,fit_stau1,fit_sa2,fit_smu2,fit_ssig2,fit_stau2,fit_sa3,fit_mu3,fit_ssig3,fit_sa4,fit_smu4,fit_ssig4,fit_sw])
            ...
            current_mdl= model(time,fit_a1,fit_mu1,fit_sig1,fit_tau1,fit_a2,fit_mu2,fit_sig2,fit_tau2,fit_a3,fit_mu3,fit_sig3,fit_a4,fit_mu4,fit_sig4,fit_w)[0:s_l]
            ...
            if plotter:
                fig,ax = plt.subplots(1,1,figsize=(f_w,10))
                ...
                ax.plot(model(time,fit_a1,fit_mu1,fit_sig1,fit_tau1,0,fit_mu2,fit_sig2,fit_tau2,0,fit_mu3,fit_sig3,0,fit_mu4,fit_sig4,fit_w), label="Fit Flux P1")
                ax.plot(model(time,0,fit_mu1,fit_sig1,fit_tau1,fit_a2,fit_mu2,fit_sig2,fit_tau2,0,fit_mu3,fit_sig3,0,fit_mu4,fit_sig4,fit_w), label="Fit Flux P2")
                ax.plot(model(time,0,fit_mu1,fit_sig1,fit_tau1,0,fit_mu2,fit_sig2,fit_tau2,fit_a3,fit_mu3,fit_sig3,0,fit_mu4,fit_sig4,fit_w), label="Fit Flux P3")
                ax.plot(model(time,0,fit_mu1,fit_sig1,fit_tau1,0,fit_mu2,fit_sig2,fit_tau2,0,fit_mu3,fit_sig3,fit_a4,fit_mu4,fit_sig4,fit_w), label="Fit Flux P4")
                ...
                plt.savefig(pref+'4_ pulse 4_mixed'+str(fl[f])+  f_unit +'.png')
                plt.close()
        except:
            result[:,f-f_i]= np.nan
        op=1
    ...
    if plotter:
        try:
            ...
            plt.savefig(pref+'_4_pulse_4_mix_Parameter_Panels_.png')
            plt.close()


            parms=[["freqs"],["fit_a1"], ["fit_mu1"], ["fit_sig1"], ["fit_tau1"],["fit_a2"], ["fit_mu2"], ["fit_sig2"], ["fit_tau2"], ["fit_a3"], ["fit_mu3"], ["fit_sig3"],["fit_a4"], ["fit_mu4"], ["fit_sig4"], ["fit_w"],["e_fit_a1"], ["e_fit_mu1"], ["e_fit_sig1"], ["e_fit_tau1"],["e_fit_a2"], ["e_fit_mu2"], ["e_fit_sig2"], ["e_fit_tau2"], ["e_fit_a3"], ["e_fit_mu3"], ["e_fit_sig3"], ["e_fit_a4"], ["e_fit_mu4"], ["e_fit_sig4"], ["e_fit_w"],['reduced Xi^2']]
