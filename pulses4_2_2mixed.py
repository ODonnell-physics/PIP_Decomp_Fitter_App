# -*- coding: utf-8 -*-
"""
Created on Thu Jan 13 10:57:41 2022
...
def model(t, a1, mu1, sig1, tau1, a2, mu2, sig2, tau2, a3, mu3, sig3, tau3, a4, mu4, sig4, tau4, w1, w2):
    ...
    decay_fast1 = np.exp(-(t) / tau1)
    decay_slow1 = np.exp(-(t) / tau2)
    ...
    decay_fast2 = np.exp(-(t) / tau3)
    decay_slow2 = np.exp(-(t) / tau4)
...
def p4_2_2m(m_s,m_s_e,freqs, p_times,f_i,f_f,t_i,t_f,ig,ub,lb, plotter=True, f_unit="GHz", b_unit="SFU", t_unit="s",log_scale=False):
    ...
    result=np.zeros([18,f_w])
    result_error=np.zeros([18,f_w])
    ...
    if plotter:
        if not os.path.exists('./4_pulses_2_2_mixed_fit'):
            os.makedirs('./4_pulses_2_2_mixed_fit')

        pref = './4_pulses_2_2_mixed_fit/4_2_2'
   #import pdb
    oddflag= False
    ...
    for f in range(f_i, f_f, 1):
        ...
        try:
            ...
            results=np.array([fit_a1,fit_mu1,fit_sig1,fit_tau1,fit_a2,fit_mu2,fit_sig2,fit_tau2,fit_a3,fit_mu3,fit_sig3,fit_tau3,fit_a4,fit_mu4,fit_sig4,fit_tau4, fit_w1,fit_w2])
            results_error= np.array([fit_sa1,fit_smu1,fit_ssig1,fit_stau1,fit_sa2,fit_smu2,fit_ssig2,fit_stau2,fit_sa3,fit_smu3,fit_ssig3,fit_stau3,fit_sa4,fit_smu4,fit_ssig4,fit_stau4, fit_w1,fit_w2])
            ...
            decay_fast1=np.exp(-(time)/fit_tau1)
            decay_slow1=np.exp(-(time)/fit_tau2)
            ...
            decay_fast2=np.exp(-(time)/fit_tau3)
            decay_slow2=np.exp(-(time)/fit_tau4)
            ...
            current_mdl= model(time,fit_a1,fit_mu1,fit_sig1,fit_tau1,fit_a2,fit_mu2,fit_sig2,fit_tau2,fit_a3,fit_mu3,fit_sig3,fit_tau3,fit_a4,fit_mu4,fit_sig4,fit_tau4,fit_w1,fit_w2)[0:s_l]
            ...
            if plotter:
                ...
                ax.plot(model(time,fit_a1,fit_mu1,fit_sig1,fit_tau1,0,fit_mu2,fit_sig2,fit_tau2,0,fit_mu3,fit_sig3,fit_tau3,0,fit_mu4,fit_sig4,fit_tau4,fit_w1,fit_w2), label="Fit Flux P1")
                ax.plot(model(time,0,fit_mu1,fit_sig1,fit_tau1,fit_a2,fit_mu2,fit_sig2,fit_tau2,0,fit_mu3,fit_sig3,fit_tau3,0,fit_mu4,fit_sig4,fit_tau4,fit_w1,fit_w2), label="Fit Flux P2")
                ax.plot(model(time,0,fit_mu1,fit_sig1,fit_tau1,0,fit_mu2,fit_sig2,fit_tau2,fit_a3,fit_mu3,fit_sig3,fit_tau3,0,fit_mu4,fit_sig4,fit_tau4,fit_w1,fit_w2), label="Fit Flux P3")
                ax.plot(model(time,0,fit_mu1,fit_sig1,fit_tau1,0,fit_mu2,fit_sig2,fit_tau2,0,fit_mu3,fit_sig3,fit_tau3,fit_a4,fit_mu4,fit_sig4,fit_tau4,fit_w1,fit_w2), label="Fit Flux P4")
                ...
                plt.savefig(pref+'_4 pulse Sample_'+str(fl[f])+ ''+ f_unit +'.png')
                plt.close()
        except:
            result[:,f-f_i]= np.nan
    ...
    if plotter:
        try:
            ...
            plt.savefig(pref+'_4_pulses_2_2_mix_Parameter_Panels_.png')
            plt.close()

            parms=[["freqs"],["fit_a1"], ["fit_mu1"], ["fit_sig1"], ["fit_tau1"],["fit_a2"], ["fit_mu2"], ["fit_sig2"], ["fit_tau2"], ["fit_a3"], ["fit_mu3"], ["fit_sig3"], ["fit_tau3"],["fit_a4"], ["fit_mu4"], ["fit_sig4"], ["fit_tau4"],["fit_w1"],["fit_w2"],["e_fit_a1"], ["e_fit_mu1"], ["e_fit_sig1"], ["e_fit_tau1"],["e_fit_a2"], ["e_fit_mu2"], ["e_fit_sig2"], ["e_fit_tau2"], ["e_fit_a3"], ["e_fit_mu3"], ["e_fit_sig3"], ["e_fit_tau3"],["e_fit_a4"], ["e_fit_mu4"], ["e_fit_sig4"], ["e_fit_tau4"],["e_fit_w_1"],["e_fit_w_2"],['reduced Xi^2']]
