...
def model(t, a1, mu1, sig1, tau1, a2, mu2, sig2, tau2, a3, mu3, sig3, w):
    try:
        measured_signal2
    except:
    ...
    test_signal = (flux1 + flux2 + flux3)[0:t_l] + measured_signal2[0]

    return test_signal


def p3_3m(m_s,m_s_e,freqs,p_times,f_i,f_f,t_i,t_f,ig,ub,lb, plotter=True, f_unit="GHz", b_unit="SFU", t_unit="s", log_scale=False, rolling_ig=False):
    ...
    result=np.zeros([12,f_w])
    result_error=np.zeros([12,f_w])
    ...
    for f in range(f_i, f_f, 1):
        ...
        try:
            ...
            fit_a1,fit_mu1,fit_sig1,fit_tau1,fit_a2,fit_mu2,fit_sig2,fit_tau2,fit_a3,fit_mu3,fit_sig3, fit_w = ans
            fit_sa1,fit_smu1,fit_ssig1,fit_stau1,fit_sa2,fit_smu2,fit_ssig2,fit_stau2,fit_sa3,fit_smu3,fit_ssig3, fit_sw = np.sqrt(np.diag(cov))
        
            
        
            results=np.array([fit_a1,fit_mu1,fit_sig1,fit_tau1,fit_a2,fit_mu2,fit_sig2,fit_tau2,fit_a3,fit_mu3,fit_sig3, fit_w])
            results_error= np.array([fit_sa1,fit_smu1,fit_ssig1,fit_stau1,fit_sa2,fit_smu2,fit_ssig2,fit_stau2,fit_sa3,fit_smu3,fit_ssig3, fit_sw])
            ...
            if plotter:
                ...
                ax.plot(model(time,fit_a1,fit_mu1,fit_sig1,fit_tau1,0,fit_mu2,fit_sig2,fit_tau2,0,fit_mu3,fit_sig3,fit_w), label="Fit Flux P1",color='darkorange')
                ax.plot(model(time,0,fit_mu1,fit_sig1,fit_tau1,fit_a2,fit_mu2,fit_sig2,fit_tau2,0,fit_mu3,fit_sig3,fit_w), label="Fit Flux P2", color='navy')
                ax.plot(model(time,0,fit_mu1,fit_sig1,fit_tau1,0,fit_mu2,fit_sig2,fit_tau2,fit_a3,fit_mu3,fit_sig3,fit_w), label="Fit Flux P3", color='darkgreen')
    ...
    if plotter:
        try:
            ...
            plt.savefig(pref+'_pulse3_3mixedParameter_Panels_.png')
            plt.close()

            parms=[["freqs"],["fit_a1"], ["fit_mu1"], ["fit_sig1"], ["fit_tau1"],["fit_a2"], ["fit_mu2"], ["fit_sig2"], ["fit_tau2"], ["fit_a3"], ["fit_mu3"], ["fit_sig3"], ["fit_w"],["e_fit_a1"], ["e_fit_mu1"], ["e_fit_sig1"], ["e_fit_tau1"],["e_fit_a2"], ["e_fit_mu2"], ["e_fit_sig2"], ["e_fit_tau2"], ["e_fit_a3"], ["e_fit_mu3"], ["e_fit_sig3"], ["e_fit_w"],['reduced Xi^2']]
